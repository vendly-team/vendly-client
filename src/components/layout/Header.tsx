...
import { Search, ShoppingCart, User, Heart, Menu, X, ChevronDown, LogOut, Package, Shield } from "lucide-react";
...
const Header = () => {
  ...
  return (
    <header className={`sticky top-0 z-50 border-b transition-all duration-300 ${isMiniApp ? 'pt-24' : ''} ${scrolled ? 'bg-background/70 backdrop-blur-xl backdrop-saturate-150 border-black/[0.06] shadow-[0_4px_24px_rgba(0,0,0,0.06)]' : 'bg-card border-border'}`}>
      <div className="container flex items-center gap-4 py-3">
        ...
        <div className="flex items-center gap-1 ml-auto">
          ...
          <Link to="/cart" className="p-2 text-foreground hover:text-accent transition-colors relative">
            ...
            {totalItems > 0 && <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] rounded-full bg-accent text-accent-foreground text-[10px] font-bold tracking-[-0.005em] flex items-center justify-center">{totalItems}</span>}
          </Link>

          {/* Profile dropdown — handles both auth and non-auth */}
          <div className="relative" ref={profileRef}>
            ...
            >
              ...
              {isAuthenticated && (
                <>
                  <span className="hidden md:inline text-[14px] font-medium tracking-[-0.011em]">{user?.firstName}</span>
              ...
              <div className="absolute right-0 top-full mt-1 w-52 bg-card border border-border rounded-lg shadow-lg py-1 z-50 animate-fade-in">
                {isAuthenticated ? (
                  <>
                    ...
                    <LangSegmented />
                    <hr className="my-1 border-border" />
                    <button onClick={() => { logout(); setProfileOpen(false); navigate('/'); }} className="flex items-center gap-2 px-4 py-2 text-[14px] font-medium tracking-[-0.006em] text-destructive hover:bg-muted w-full"><LogOut size={16} /> {t("common.signOut")}</button>
                  </>
                ) : (
                  <>
      ...
      <nav
        className={`hidden md:grid border-t transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] overflow-hidden ${showCategoryNav
          ? 'grid-rows-[1fr] opacity-100 border-border/50'
          : 'grid-rows-[0fr] opacity-0 border-transparent'
          }`}
      >
        <div className="overflow-hidden">
          <div className={`container flex items-center gap-6 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${showCategoryNav ? 'py-2 translate-y-0' : 'py-0 -translate-y-2'
            } overflow-x-auto`}>
            {categories.map((cat, i) => (
              <Link
                key={cat.id}
                to={`/category/${cat.slug}`}
                className={`text-[14px] tracking-[-0.011em] text-foreground/80 hover:text-accent whitespace-nowrap font-medium transition-all duration-500 ${showCategoryNav
    ...
    mobileMenuOpen && (
      <div className="md:hidden absolute top-full left-0 w-full bg-card border-b border-border shadow-lg animate-fade-in z-40">
        <nav className="container py-4 space-y-1">
          {categories.map((cat) => (
            <Link key={cat.id} to={`/category/${cat.slug}`} className="block py-2.5 px-3 text-[14px] font-medium tracking-[-0.011em] text-foreground hover:bg-muted rounded-md transition-colors" onClick={() => setMobileMenuOpen(false)}>
              {cat.name}
            </Link>
          ))}
        </nav>
