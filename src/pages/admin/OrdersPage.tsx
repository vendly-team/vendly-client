...
const AdminOrdersPage = () => {
  ...
  return (
    <div>
      <h1 className="text-2xl font-display font-bold mb-6">{t('orders.title')}</h1>
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="relative"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input placeholder={t('orders.searchPlaceholder')} value={search} onChange={e => setSearch(e.target.value)} className="h-9 pl-9 pr-3 glass-input rounded-md text-sm w-52" /></div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="h-9 px-3 glass-input rounded-md text-sm"><option value="">{t('orders.allStatus')}</option><option value="new">{t('orders.new')}</option><option value="accepted">{t('orders.accepted')}</option><option value="in_transit">{t('orders.inTransit')}</option><option value="delivered">{t('orders.delivered')}</option><option value="cancelled">{t('orders.cancelled')}</option></select>
      </div>
      <div className="bg-card border border-border rounded-lg overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border bg-muted/50"><th className="text-left px-4 py-3 font-medium text-muted-foreground">{t('orders.orderNumber')}</th><th className="text-left px-4 py-3 font-medium text-muted-foreground">{t('common.customer')}</th><th className="text-left px-4 py-3 font-medium text-muted-foreground">{t('common.date')}</th><th className="text-left px-4 py-3 font-medium text-muted-foreground">{t('common.status')}</th><th className="text-left px-4 py-3 font-medium text-muted-foreground">{t('common.payment')}</th><th className="text-right px-4 py-3 font-medium text-muted-foreground">{t('common.total')}</th><th className="text-left px-4 py-3 font-medium text-muted-foreground">{t('common.actions')}</th></tr></thead>
          <tbody>
            {paged.map(o => {
              ...
              return (
                <tr key={o.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3"><Link to={`/admin/orders/${o.id}`} className="text-accent hover:underline text-xs">{t('common.view')}</Link></td>
                </tr>
              );
            })}
          </tbody>
