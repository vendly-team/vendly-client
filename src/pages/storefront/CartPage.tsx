    ...
    return (
      <StorefrontLayout>
        <div className="container py-20 text-center animate-fade-in">
          ...
          <Link to="/" className="inline-flex h-11 px-8 items-center rounded-lg bg-accent text-accent-foreground font-semibold text-sm">{t('cart.continueShopping')}</Link>
        </div>
      </StorefrontLayout>
    );
  }
  ...
  return (
    <StorefrontLayout>
      <div className="container py-6 animate-fade-in">
        <h1 className="text-2xl font-display font-bold text-foreground mb-6">{t('cart.title', { count: totalItems })}</h1>
          ...
          <div className="lg:w-80">
            <div className="bg-card border border-border rounded-lg p-6 sticky top-24">
              ...
              <div className="space-y-2 text-sm mb-4">
                <div className="flex justify-between"><span className="text-muted-foreground">{t('common.subtotal')}</span><span className="font-medium">{formatPrice(totalAmount)}</span></div>
                <p className="text-xs text-muted-foreground">{t('cart.deliveryHint')}</p>
              </div>
              <hr className="border-border mb-4" />
              <div className="flex justify-between font-semibold text-foreground mb-4">
                <span>{t('common.total')}</span><span>{formatPrice(totalAmount)}</span>
              </div>
