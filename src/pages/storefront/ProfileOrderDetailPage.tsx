...
const ProfileOrderDetailPage = () => {
  ...
  if (!order) return <div className="py-20 text-center"><h2 className="text-xl font-bold">{t('orders.notFound')}</h2><Link to="/profile/orders" className="text-accent">{t('profileOrders.backToOrders')}</Link></div>;
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <span className={`text-xs font-medium px-2 py-1 rounded ${statusColors[order.status] || ''}`}>{t(stepLabelKeys[order.status] || 'statusLabels.cancelled')}</span>
      </div>
        ...
        <div className="flex items-center gap-2 mb-8 overflow-x-auto">
          {steps.map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              ...
              <span className={`text-sm whitespace-nowrap ${i <= currentStepIndex ? 'text-foreground font-medium' : 'text-muted-foreground'}`}>{t(stepLabelKeys[s])}</span>
              {i < steps.length - 1 && <div className={`w-8 h-0.5 ${i < currentStepIndex ? 'bg-accent' : 'bg-border'}`} />}
            </div>
          ))}
        </div>
      ) : (
        <div className="bg-destructive/10 text-destructive px-4 py-3 rounded-lg mb-6 text-sm">{t('orders.wasCancelled')}</div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-card border border-border rounded-lg p-4">
          <h3 className="font-semibold mb-2">{t('common.delivery')}</h3>
          ...
          <p className="text-sm text-muted-foreground mt-2">{t('profileOrders.estDelivery')} {order.estimatedDelivery}</p>
        </div>
        <div className="bg-card border border-border rounded-lg p-4">
          <h3 className="font-semibold mb-2">{t('common.payment')}</h3>
          <p className="text-sm text-muted-foreground">{t('profileOrders.method')} {order.paymentProvider || 'N/A'}</p>
          <p className="text-sm text-muted-foreground">{t('common.status')}: {order.paymentStatus}</p>
          <hr className="my-2 border-border" />
          <div className="flex justify-between text-sm"><span>{t('common.subtotal')}</span><span>{formatPrice(order.totalAmount - order.deliveryCost)}</span></div>
          <div className="flex justify-between text-sm"><span>{t('common.delivery')}</span><span>{formatPrice(order.deliveryCost)}</span></div>
          <div className="flex justify-between font-bold mt-1"><span>{t('common.total')}</span><span>{formatPrice(order.totalAmount)}</span></div>
        </div>
      </div>
    </div>
  );
