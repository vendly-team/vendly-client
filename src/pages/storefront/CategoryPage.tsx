...
const CategoryPage = () => {
  ...
  const FilterPanel = () => (
    <div className="space-y-4">
      <h3 className="font-semibold text-foreground">{t('categoryPage.filters')}</h3>
      <div>
        <label className="text-sm font-medium text-foreground">{t('categoryPage.priceRange')}</label>
        <div className="flex gap-2 mt-1">
          <input type="number" placeholder={t('categoryPage.min')} value={priceMin} onChange={(e) => { setPriceMin(e.target.value); setPage(1); }} className="w-full h-9 px-2 border border-border rounded text-sm bg-background" />
          <input type="number" placeholder={t('categoryPage.max')} value={priceMax} onChange={(e) => { setPriceMax(e.target.value); setPage(1); }} className="w-full h-9 px-2 border border-border rounded text-sm bg-background" />
        </div>
      </div>
      ...
      <button onClick={resetFilters} className="text-sm text-accent hover:underline">{t('categoryPage.resetFilters')}</button>
    </div>
  );
    ...
    <StorefrontLayout>
      <div className="container py-6 animate-fade-in">
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
          ...
          <span className="text-foreground">{category?.name || t('categoryPage.allProducts')}</span>
        </div>
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-display font-bold text-foreground">{category?.name || t('categoryPage.allProducts')}</h1>
          ...
          <div className="flex-1">
            {paged.length > 0 ? (
              <>
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 mt-8">
                    <button disabled={page === 1} onClick={() => setPage(page - 1)} className="h-9 px-3 border border-border rounded text-sm disabled:opacity-50">{t('common.prev')}</button>
                    ...
                    <button disabled={page === totalPages} onClick={() => setPage(page + 1)} className="h-9 px-3 border border-border rounded text-sm disabled:opacity-50">{t('common.next')}</button>
                  </div>
                )}
              </>
            ) : (
              <div className="text-center py-20">
                ...
                <h3 className="text-lg font-semibold text-foreground mb-2">{t('categoryPage.noProducts')}</h3>
                <button onClick={resetFilters} className="text-accent hover:underline">{t('categoryPage.resetFilters')}</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-foreground/50 md:hidden" onClick={() => setMobileFiltersOpen(false)}>
          <div className="absolute right-0 top-0 h-full w-72 bg-card p-6 animate-slide-in-right" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold">{t('categoryPage.filters')}</h3>
