    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        {['all','pending','approved','rejected'].map(f => (
          <button key={f} onClick={() => setFilter(f)} className={`h-9 px-4 rounded-md text-sm font-medium ${filter === f ? 'bg-accent text-accent-foreground' : 'border border-border'}`}>{f.charAt(0).toUpperCase() + f.slice(1)}</button>
        ))}
      </div>
      {selected.length > 0 && (
        <div className="flex gap-2 mb-4 bg-muted p-3 rounded-lg">
          <span className="text-sm">{selected.length} selected</span>
          <button onClick={() => updateStatus(selected, 'approved')} className="h-8 px-3 bg-success/10 text-success rounded text-xs font-medium">Approve Selected</button>
          <button onClick={() => updateStatus(selected, 'rejected')} className="h-8 px-3 bg-destructive/10 text-destructive rounded text-xs font-medium">Reject Selected</button>
        </div>
      )}
              ...
              return (
                <tr key={r.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 flex gap-1">
                    <button onClick={() => updateStatus([r.id], 'approved')} className="w-7 h-7 rounded bg-success/10 text-success flex items-center justify-center" title="Approve"><Check size={14} /></button>
                    <button onClick={() => updateStatus([r.id], 'rejected')} className="w-7 h-7 rounded bg-destructive/10 text-destructive flex items-center justify-center" title="Reject"><X size={14} /></button>
                  </td>
                </tr>
              );
            })}
